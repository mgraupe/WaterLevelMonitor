    ...
    def readTSL2591All(self):
        for m in range(len(self.gains)):
            ...
            for n in range(len(self.reps)): # repeat each intergration time twice, sensor exhibited delayed setting responses
                ...
                #time.sleep(1.)
                print('gain rep %s, integration time rep %s' % (m,n))
                print('gain : ', self.sensor.gain)
                print('integration time : ', self.sensor.integration_time)
                try:
                    ...
                    if self.reps[n][1]<2:
                        ...
                        break # abort integration time loop in case reading was successful
                #print("Full spectrum (IR + visible) light: {0}".format(full_spectrum))
            if lux == 0.:
                pass # continue gain loop with increasing gain in case reading is zero
            else:
                break # stop gain loop in case luminosity was non-zero
        #self.sensor.disable()
        #self.i2c.deinit()
